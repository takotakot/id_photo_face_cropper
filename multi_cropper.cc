...
std::string result_file_name = "result_crop.txt";
...
struct cropper{
	...
	void crop_writeout(std::string &write_img_name, cv::Mat &img_color, face_detect_result &result) {
		std::vector<cv::Rect>::const_iterator r;
		r = select_intelligent(faces_all);

		cv::Point tl_cut, br_cut;
		tl_cut.x = (result.rx - result.rw * 0.325);
		tl_cut.y = (result.ry - result.rh * 0.6);
		br_cut.x = (result.rx + result.rw * 1.325);
		br_cut.y = (result.ry + result.rh * 1.6);
	...
	*/
	...
	#pragma omp parallel
	{
		...
		for(int i = 0; i < file_list.size(); ++i) {
			...
			// write
			cropper_obj.crop_writeout(write_img_name, img_color, results[i]);
