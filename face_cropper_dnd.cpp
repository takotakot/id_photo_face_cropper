...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2) {
    // nop
    std::cout
        << "Give command line arguements or Drag and Drop files or directories."
        << std::endl;
    return 0;
  } else {
    for (int i = 1; i < argc; ++i) {
      ...
      ssize_t size;
      char *posix;
      size = cygwin_conv_path(CCP_WIN_A_TO_POSIX | CCP_PROC_CYGDRIVE,
                              dir_str.c_str(), NULL, 0);
      if (size < 0) {
        perror("cygwin_conv_path");
      } else {
        posix = (char *)malloc(size);
        if (cygwin_conv_path(CCP_WIN_A_TO_POSIX | CCP_PROC_CYGDRIVE,
                             dir_str.c_str(), posix, size))
          perror("cygwin_conv_path");
      }
      dir_str = posix;
...
// #pragma omp parallel for private(read_img_name, write_img_name, img_color,
// recog_ok, cropper_obj) private(read_img_name, write_img_name, img_color,
// recog_ok)
...
#pragma omp critical
          {
            std::cerr << "th" << thread_num << ": " << read_img_name
                      << std::endl;
          }
          write_img_name = append_date_suffix(read_img_name, date_suffix);

          read_recognize_error =
              cropper.detect_and_output(read_img_name, write_img_name, oss);
#pragma omp critical
          {
...
#pragma omp critical
          {
            std::cerr << "th" << thread_num << ": " << src.dir.filelist[i]
                      << std::endl;
          }
