...
cv::RotatedRect RotatedRect_pt(const cv::Point2d &_point1,
                               const cv::Point2d &_point2,
                               const cv::Point2d &_point3) {
  ...
  double _angle =
      std::atan(vecs[wd_i][1] / vecs[wd_i][0]) * 180.0f / (double)CV_PI;
...
face_metrics::face_metrics(double focal_length, cv::Point2d center,
                           dlib::full_object_detection &shape)
    : focal_length(focal_length), center(center) {
  ...
  double dx = shape.part(16).x() - shape.part(0).x(),
         dy = shape.part(16).y() - shape.part(0).y();
...
type_point face_metrics::coordsOf(dlib::full_object_detection &shape,
                                  FACIAL_FEATURE feature) {
  return toCv(shape.part(feature));
}

cv::Mat face_metrics::get_points_index(int index[], const int &n_index,
                                       dlib::full_object_detection &shape) {
...
cv::Mat face_metrics::get_center_points(dlib::full_object_detection &shape) {
  ...
  const int n_center_points =
      sizeof(center_points_index) / sizeof(center_points_index[0]);
...
cv::Mat face_metrics::get_eye_points(dlib::full_object_detection &shape) {
  ...
  const int n_eye_points =
      sizeof(eye_points_index) / sizeof(eye_points_index[0]);
...
void face_metrics::add_debug_image(cv::Mat &image) { _debug = image.clone(); }
...
head_pose face_metrics::calc_pose(dlib::full_object_detection &shape) {
  ...
  auto stomion = (coordsOf(shape, MOUTH_CENTER_TOP) +
                  coordsOf(shape, MOUTH_CENTER_BOTTOM)) *
                 0.5;
  ...
  cv::solvePnP(head_points, detected_points, projection, cv::noArray(),
  ...
  head_pose pose = {rotation(0, 0),
                    rotation(0, 1),
                    rotation(0, 2),
                    translation_vector.at<double>(0) / 1000,
                    rotation(1, 0),
                    rotation(1, 1),
                    rotation(1, 2),
                    translation_vector.at<double>(1) / 1000,
                    rotation(2, 0),
                    rotation(2, 1),
                    rotation(2, 2),
                    translation_vector.at<double>(2) / 1000,
                    0,
                    0,
                    0,
                    1};
  ...
  cv::projectPoints(head_points, rotation_vector, translation_vector,
                    projection, cv::noArray(), reprojected_points);
  ...
  cv::projectPoints(axes, rotation_vector, translation_vector, projection,
                    cv::noArray(), projected_axes);
  std::cerr << omp_get_thread_num() << "\t" << __LINE__ << std::endl;
  cv::line(_debug, projected_axes[0], projected_axes[3], cv::Scalar(255, 0, 0),
           2, CV_AA);
  cv::line(_debug, projected_axes[0], projected_axes[2], cv::Scalar(0, 255, 0),
           2, CV_AA);
  cv::line(_debug, projected_axes[0], projected_axes[1], cv::Scalar(0, 0, 255),
           2, CV_AA);

  // putText(_debug, "(" + to_string(int(pose(0, 3) * 100)) + "cm, " +
  // to_string(int(pose(1, 3) * 100)) + "cm, " + to_string(int(pose(2, 3) *
  // 100)) + "cm)", coordsOf(shape, SELLION), FONT_HERSHEY_SIMPLEX, 0.5,
  // cv::Scalar(0, 0, 255), 2);
#endif
  std::cerr << omp_get_thread_num() << "\t" << __LINE__ << std::endl;
  ...
  std::cerr << omp_get_thread_num() << "\t" << __LINE__ << std::endl;
  decomposeProjectionMatrix(projection_matrix, projection, rotation,
                            translation_vector, cv::noArray(), cv::noArray(),
                            cv::noArray(), eulerAngles);
  yaw = eulerAngles[1];
  pitch = eulerAngles[0];
  roll = eulerAngles[2];
...
template void face_cropper::detect<dlib::bgr_pixel>(
    dlib::cv_image<dlib::bgr_pixel> &);

int face_cropper::get_num_faces() { return faces.size(); }
...
-0.05133210573416831, 0.9986816384218227, 67.81152397893035]
  */
  cv::warpAffine(i_img, rotated, rotation_matrix, rotated.size(),
                 cv::INTER_CUBIC);
  // 回転した画像から矩形領域を切り出す
...
bool face_cropper::detect_and_output(std::string &read_img_name,
                                     std::string &write_img_name,
