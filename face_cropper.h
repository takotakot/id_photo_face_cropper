...
const static double THETA_0 =
    M_2_PI -
    std::acos((121.1 * 121.1 + 53.0 * 53.0 - 71.4 * 71.4) / (2 * 121.1 * 53.0));
...
struct face_metrics {
  ...
  face_metrics(double focal_length, cv::Point2d center,
               dlib::full_object_detection &shape);
  type_point coordsOf(dlib::full_object_detection &shape,
                      FACIAL_FEATURE feature);
  cv::Mat get_points_index(int index[], const int &n_index,
                           dlib::full_object_detection &shape);
